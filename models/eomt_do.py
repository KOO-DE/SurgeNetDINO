...
class EoMT(nn.Module):
    def __init__(
        ...
        mc_dropout_p: float = 0.1,   # Dropout 확률 (학습 시 regularization, MC Dropout inference 시 uncertainty 측정)
    ):
        super().__init__()
        ...
        # Dropout: 학습 시 regularization / MC Dropout inference 시 uncertainty 측정
    ...
    def _apply_mc_dropout(self, x: torch.Tensor) -> torch.Tensor:
        """
        학습 시 (self.training=True): 일반 dropout으로 regularization
        MC Dropout inference 시 (self.training=False): 강제로 train 모드로 켜서 dropout 적용
        일반 inference 시 (self.training=False, mc_dropout.p=0): 항등함수
        """
        if not self.training and self.mc_dropout.p > 0:
            # MC Dropout inference: eval 모드에서 강제로 dropout 적용
            self.mc_dropout.train()
            out = self.mc_dropout(x)
            self.mc_dropout.eval()
            return out
        # 학습 시: nn.Dropout이 self.training=True를 보고 정상적으로 dropout 적용
    ...
    def forward(self, x: torch.Tensor):
        ...
        for i, block in enumerate(self.encoder.backbone.blocks):
            ...
            # 마지막 블록 직전 단일 위치에 dropout 적용 (학습 시 비활성)
            if i == len(self.encoder.backbone.blocks) - self.num_blocks - 7:
                x = self._apply_mc_dropout(x)
