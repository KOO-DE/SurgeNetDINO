...
class LightningModule(lightning.LightningModule):
    ...
    def init_metrics_semantic(self, ignore_idx, num_blocks):
        ...
        self.metrics_acc = nn.ModuleList(
            [
                MulticlassAccuracy(
                    num_classes=self.num_classes,
                    validate_args=False,
                    ignore_index=ignore_idx,
                    average="micro",  # pixel accuracy
                )
                for _ in range(num_blocks)
            ]
        )
    ...
    def update_metrics_semantic(
        ...
        for i in range(len(preds)):
            ...
            self.metrics_acc[block_idx].update(preds[i][None, ...], targets[i][None, ...])
    ...
    def _on_eval_epoch_end_semantic(self, log_prefix, log_per_class=False):
        for i, (metric, acc_metric) in enumerate(zip(self.metrics, self.metrics_acc)):  # type: ignore
            iou_per_class = metric.compute()
            metric.reset()
            pixel_acc = acc_metric.compute()
            acc_metric.reset()
            ...
            self.log(
                f"metrics/{log_prefix}_pixel_acc{block_postfix}",
                float(pixel_acc),
            )
            iou_fg = iou_per_class[1:]
            dice_fg = 2 * iou_fg / (1 + iou_fg)
            dice_fg_mean = float(dice_fg.mean())
            self.log(
                f"metrics/{log_prefix}_dice_fg{block_postfix}",
                dice_fg_mean,
            )
    ...
    def _on_eval_end_semantic(self, log_prefix):
        if not self.trainer.sanity_checking:
            rank_zero_info(
                f"{bold_green}mIoU: {self.trainer.callback_metrics[f'metrics/{log_prefix}_iou_all'] * 100:.1f} | "
                f"Pixel Acc: {self.trainer.callback_metrics[f'metrics/{log_prefix}_pixel_acc'] * 100:.1f}{reset}"
            )
    ...
    def plot_semantic(
        ...
        if self.trainer.logger is not None and hasattr(self.trainer.logger.experiment, "log"):
    ...
    def _load_ckpt(self, ckpt_path, load_ckpt_class_head):
        ...
        # pos_embed interpolation: checkpoint 해상도와 현재 모델 해상도가 다를 때 처리
        # SurgeNetDINO는 336x336으로 pretraining → 504x504 fine-tuning 시 필요
        pos_embed_key = None
        for k in ckpt.keys():
            if "pos_embed" in k and "backbone" in k:
                pos_embed_key = k
                break
        if pos_embed_key is not None:
            current_state = self.state_dict()
            if pos_embed_key in current_state:
                ckpt_pos = ckpt[pos_embed_key]        # [1, N_ckpt, C]
                cur_pos = current_state[pos_embed_key] # [1, N_cur, C]
                if ckpt_pos.shape != cur_pos.shape:
                    logging.info(
                        f"pos_embed shape mismatch: {ckpt_pos.shape} → {cur_pos.shape}, interpolating..."
                    )
                    # cls token 분리
                    cls_token = ckpt_pos[:, :1, :]          # [1, 1, C]
                    patch_tokens = ckpt_pos[:, 1:, :]        # [1, N-1, C]

                    # 현재 모델의 grid size 계산
                    cur_n = cur_pos.shape[1] - 1             # cls token 제외
                    cur_grid = int(cur_n ** 0.5)

                    # checkpoint grid size 계산
                    ckpt_n = patch_tokens.shape[1]
                    ckpt_grid = int(ckpt_n ** 0.5)

                    # 2D interpolation
                    patch_tokens = patch_tokens.reshape(1, ckpt_grid, ckpt_grid, -1)
                    patch_tokens = patch_tokens.permute(0, 3, 1, 2)  # [1, C, H, W]
                    patch_tokens = torch.nn.functional.interpolate(
                        patch_tokens.float(),
                        size=(cur_grid, cur_grid),
                        mode="bicubic",
                        align_corners=False,
                    ).to(ckpt_pos.dtype)
                    patch_tokens = patch_tokens.permute(0, 2, 3, 1).reshape(1, -1, ckpt_pos.shape[-1])

                    ckpt[pos_embed_key] = torch.cat([cls_token, patch_tokens], dim=1)
                    logging.info(f"pos_embed interpolated: {ckpt_pos.shape} → {ckpt[pos_embed_key].shape}")

        logging.info(f"Loaded {len(ckpt)} keys")
        return ckpt

    def _raise_on_incompatible(self, incompatible_keys, load_ckpt_class_head):
        # backbone only weights (e.g. SurgeNetDINO)를 로드할 때
        # EoMT head 관련 키들은 checkpoint에 없는 게 정상 → missing 허용
        EOMT_HEAD_KEYS = (
            "network.q.",
            "network.mask_head.",
            "network.upscale.",
        ...
        if incompatible_keys.missing_keys:
            missing_keys = [
                ...
                if not any(key.startswith(h) or key == h for h in EOMT_HEAD_KEYS)
            ]
            if not load_ckpt_class_head:
                missing_keys = [
        ...
        # pretraining 전용 키들 (dino_head, ibot_head, register_tokens, mask_token 등)은
        # fine-tuning 모델에 없는 게 정상 → unexpected 허용
        PRETRAIN_ONLY_KEYS = (
            "dino_head.",
            "ibot_head.",
            "register_tokens",
        ...
        if incompatible_keys.unexpected_keys:
            unexpected_keys = [
                ...
                if not any(p in key for p in PRETRAIN_ONLY_KEYS)
            ]
            if unexpected_keys:
